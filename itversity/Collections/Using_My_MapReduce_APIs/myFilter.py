...
orders_by_customer_month = myFilter(orders, lambda n: (int(n.split(',')[2])== 12431) and (n.split(',')[1].startswith('2014-01')))
print(f'{orders_by_customer_month=}')

#Get Orders placed by customer id 12431 in the month of 2014 January and status=PENDING_PAYMENT
orders_by_customer_status = myFilter(orders, lambda n: (int(n.split(',')[2])== 12431) and 
                                                  (n.split(',')[1].startswith('2014-01')) and
                                                  (n.split(',')[3]== 'PENDING_PAYMENT') )
